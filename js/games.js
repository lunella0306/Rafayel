...
function initComboMenu() {
    ...
    function renderUserPokeMenu() {
        ...
        tabContent.style.cssText = 'flex:1;overflow-y:auto;font-family:var(--font-family);';

        wrapper.appendChild(tabContent);

        // 渲染拍一拍内容
        renderPokeContent(tabContent);
        
        function renderPokeContent(container) {
            container.innerHTML = '';
            ...
            const input = document.createElement('input');
            input.type = 'text';
            input.placeholder = `想对${partnerName}做点什么？如"拍了拍${partnerName}"`;
            input.style.cssText = `
                flex:1;padding:10px 12px;border:1px solid var(--border-color);
                border-radius:10px;background:var(--secondary-bg);color:var(--text-primary);
                font-size:13px;outline:none;font-family:var(--font-family);
            `;
            // placeholder 字体小一点
            const styleId = 'poke-input-style-games';
            if (!document.getElementById(styleId)) {
                const style = document.createElement('style');
                style.id = styleId;
                style.textContent = `#combo-content-area input::placeholder { font-size: 11px; }`;
            ...
            sendBtn.style.cssText = `
                padding:10px 14px;border:none;border-radius:10px;cursor:pointer;
                background:linear-gradient(135deg, var(--accent-color), rgba(var(--accent-color-rgb),0.8));
                color:#fff;font-size:12px;font-weight:600;font-family:var(--font-family);
                white-space:nowrap;
            `;
            
            // 重复提示
            const dupHint = document.createElement('div');
            dupHint.style.cssText = 'font-size:11px;color:#e74c3c;margin-top:6px;display:none;';
            ...
            function checkDuplicate(text) {
                ...
                const isDup = (customPokes || []).some(p => p.trim().toLowerCase() === normalized);
                if (isDup) {
                    dupHint.textContent = `⚠️ 该动作已存在`;
            ...
            input.addEventListener('input', () => checkDuplicate(input.value));
            
            // 发送拍一拍的通用函数
            function sendPokeAction(actionText) {
                const cleanText = (typeof window._sanitizePokeTextForDisplay === 'function')
                    ? window._sanitizePokeTextForDisplay(actionText)
                    : actionText;
                ...
                showNotification('已发送', 'success', 1500);
            }
            ...
            sendBtn.onclick = (e) => { 
                ...
                if (!text) {
                    showNotification('请输入动作内容', 'warning', 1500);
                    return;
                }
                sendPokeAction(text);
            };
            ...
            inputSection.appendChild(dupHint);
            ...
            presetTitle.style.cssText = 'font-size:11px;color:var(--text-secondary);margin-bottom:6px;';
            presetTitle.textContent = '快捷预设';
            ...
            presets.forEach(presetText => {
                ...
                btn.innerHTML = `<span style="margin-right:6px;opacity:0.6;">𓆟</span>${presetText}`;
                btn.style.cssText = `
                ...
                btn.onmouseover = () => {
                    ...
                    btn.style.background = 'rgba(var(--accent-color-rgb),0.08)';
                };
                btn.onmouseout = () => {
                    btn.style.borderColor = 'var(--border-color)';
                    btn.style.background = 'var(--secondary-bg)';
                };
                btn.onclick = (e) => { e.stopPropagation(); sendPokeAction(presetText); };
            ...
            divider.style.cssText = 'display:flex;align-items:center;gap:8px;margin:10px 0 8px;';
            divider.innerHTML = `
                <div style="flex:1;height:1px;background:var(--border-color);"></div>
                <span style="font-size:11px;color:var(--text-secondary);">我的自定义</span>
                <div style="flex:1;height:1px;background:var(--border-color);"></div>
            `;
            container.appendChild(divider);
            
            // ===== 自定义库区域 =====
            const userPokes = customPokes || [];
            
            if (userPokes.length === 0) {
                const emptyTip = document.createElement('div');
                emptyTip.style.cssText = `
                    text-align:center;color:var(--text-secondary);
                    padding:16px;font-size:12px;opacity:0.7;
                `;
                emptyTip.textContent = `暂无自定义动作，输入动作发送后会自动保存`;
                container.appendChild(emptyTip);
            } else {
                userPokes.forEach((text, idx) => {
                    const item = createPokeItem(text, false, true, idx);
        ...
        function showEditModal(originalText, onSave) {
            const existing = document.getElementById('poke-edit-modal');
            if (existing) existing.remove();
            
            const modal = document.createElement('div');
            modal.id = 'poke-edit-modal';
            modal.style.cssText = `
            ...
            dialog.style.cssText = `
                ...
                padding:16px;width:90%;max-width:280px;
                box-shadow:0 8px 32px rgba(0,0,0,0.2);
            `;
            
            const title = document.createElement('div');
            title.textContent = '编辑拍一拍';
            title.style.cssText = `
            ...
            saveBtn.style.cssText = `
                ...
                cursor:pointer;font-size:13px;font-weight:600;font-family:var(--font-family);
            `;
            ...
            saveBtn.onclick = (e) => {
                ...
                if (!newText) {
                    showNotification('内容不能为空', 'warning', 1500);
                    return;
                }
                modal.remove();
                onSave(newText);
        ...
        // ========== 创建拍一拍项 ==========
        // isPreset: 是否为快捷预设（预设项不可编辑删除）
        // canEditDelete: 是否可以编辑和删除
        // itemIndex: 在 customPokes 数组中的索引（用于编辑和删除）
        function createPokeItem(pokeText, isPreset, canEditDelete, itemIndex) {
            ...
            item.style.cssText = `
                ...
                padding:10px 12px;background:linear-gradient(135deg, var(--secondary-bg), rgba(var(--accent-color-rgb),0.04));
                border:1px solid rgba(var(--accent-color-rgb),0.15);border-radius:12px;
                cursor:pointer;font-size:13px;color:var(--text-primary);
                transition:all 0.2s;margin-bottom:6px;font-family:var(--font-family);
            `;

            const textSpan = document.createElement('span');
            textSpan.textContent = cleanPokeText;
            ...
            sendBtn.style.cssText = `
                ...
                background:linear-gradient(135deg, var(--accent-color), rgba(var(--accent-color-rgb),0.8));
                color:#fff;font-size:11px;font-family:var(--font-family);
            `;
            sendBtn.onclick = (e) => {
                e.stopPropagation();
                if (typeof window.addMessage === 'function') {
            ...
            if (canEditDelete && !isPreset) {
                ...
                editBtn.style.cssText = `
                    padding:6px 10px;border:none;border-radius:8px;cursor:pointer;
                    background:rgba(var(--accent-color-rgb),0.1);color:var(--accent-color);font-size:11px;font-family:var(--font-family);
                `;
                editBtn.onclick = (e) => {
                    e.stopPropagation();
                    showEditModal(cleanPokeText, (newText) => {
                        const sanitized = (typeof window._sanitizePokeTextForDisplay === 'function')
                            ? window._sanitizePokeTextForDisplay(newText)
                            : newText;
                        
                        // 检查重复（排除自己）
                        const normalized = sanitized.trim().toLowerCase();
                        const isDup = (customPokes || []).some((p, i) => 
                            i !== itemIndex && p.trim().toLowerCase() === normalized
                        );
                        if (isDup) {
                            showNotification('该动作已存在', 'warning', 2000);
                            return;
                        }
                        
                        // 更新
                        if (itemIndex !== undefined && customPokes && customPokes[itemIndex]) {
                            customPokes[itemIndex] = sanitized;
                            if (typeof throttledSaveData === 'function') throttledSaveData();
                            showNotification('已保存修改', 'success', 1500);
                            
                            // 刷新列表
                            const tabContent = document.querySelector('.poke-list-view > div:last-child');
                            if (tabContent) {
                                tabContent.innerHTML = '';
                                renderCustomTab(tabContent);
                            }
                        }
                    });
                };
                ...
                delBtn.style.cssText = `
                    padding:6px 10px;border:none;border-radius:8px;cursor:pointer;
                    background:rgba(231,76,60,0.1);color:#e74c3c;font-size:11px;font-family:var(--font-family);
                `;
                delBtn.onclick = (e) => {
                    e.stopPropagation();
                    const idx = itemIndex !== undefined ? itemIndex : customPokes.findIndex(p => p === pokeText || p === cleanPokeText);
                    if (idx !== -1) {
                        customPokes.splice(idx, 1);
                        if (typeof throttledSaveData === 'function') throttledSaveData();
                        item.remove();
                        showNotification('已删除', 'info', 1500);
