...
function showPokeTab() {
    ...
    const contentArea = document.createElement('div');
    contentArea.style.cssText = 'flex:1;overflow-y:auto;font-family:var(--font-family);';
    ...
    function renderPokeContent() {
        contentArea.innerHTML = '';
        
        // ===== 顶部输入区域 =====
        const inputWrapper = document.createElement('div');
        inputWrapper.style.cssText = 'display:flex;gap:8px;margin-bottom:8px;font-family:var(--font-family);';
        ...
        `;
        // placeholder 字体小一点
        const styleId = 'poke-input-style';
        ...
        dupHint.style.cssText = 'font-size:11px;color:#e74c3c;margin-top:-4px;margin-bottom:6px;display:none;font-family:var(--font-family);';
        ...
        contentArea.appendChild(dupHint);
        ...
        presetTitle.style.cssText = 'font-size:11px;color:var(--text-secondary);margin-bottom:6px;font-family:var(--font-family);';
        ...
        presets.forEach(presetText => {
            ...
            btn.onclick = () => sendPokeAction(presetText);
            presetSection.appendChild(btn);
        });
        
        contentArea.appendChild(presetSection);
        
        // ===== 分隔线 =====
        const divider = document.createElement('div');
        divider.style.cssText = 'display:flex;align-items:center;gap:8px;margin:10px 0 8px;font-family:var(--font-family);';
        divider.innerHTML = `
        ...
        if (customPokes.length === 0) {
            ...
            empty.style.cssText = 'text-align:center;color:var(--text-secondary);padding:16px;font-size:12px;font-family:var(--font-family);opacity:0.7;';
            empty.textContent = `暂无自定义动作，输入动作发送后会自动保存`;
            contentArea.appendChild(empty);
            return;
        }
        
        customPokes.forEach((pokeText, index) => {
            const item = createPokeItem(pokeText);
            contentArea.appendChild(item);
        });
    }
    
    // ========== 创建拍一拍项 ==========
    function createPokeItem(pokeText) {
        ...
        delBtn.onclick = (e) => {
            e.stopPropagation();
            const idx = customPokes.findIndex(p => p === pokeText || p === cleanPokeText);
            if (idx !== -1) {
        ...
        function initCoreListeners() {
            ...
            DOMElements.messageInput.addEventListener('input', () => {
                DOMElements.messageInput.style.height = 'auto'; DOMElements.messageInput.style.height = `${Math.max(32, Math.min(DOMElements.messageInput.scrollHeight, 120))}px`;
            });
