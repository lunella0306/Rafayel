...
function _getGroupCtx(tab) {
    ...
    if (tab === 'pokes') {
        ...
        // 使用 partnerPokes，这是对方触发拍一拍时使用的库
        // customPokes 是用户自己用的，存在"表情＆拍一拍"面板中
        return { groups: window.customPokeGroups, items: (typeof partnerPokes !== 'undefined' ? partnerPokes : []), itemLabel: '拍一拍' };
    }
    if (tab === 'statuses') {
...
function _renderListContentOnly() {
    ...
    if (currentMajorTab === 'reply') {
        ...
        if (currentSubTab === 'pokes') itemsToRender = (typeof partnerPokes !== 'undefined' ? partnerPokes : []);
...
function _renderAtmosphereList(list, items) {
    ...
    // 预建各数组的索引 Map，避免 O(n²)
    const indexMaps = {
        pokes:    new Map((typeof partnerPokes !== 'undefined' ? partnerPokes : []).map((r,i)   => [r, i])),
...
function _runDedup() {
    ...
    if (typeof partnerPokes !== 'undefined') {
        const cpDedup = deduplicateContentArray(partnerPokes);
        partnerPokes = cpDedup.result; totalRemoved += cpDedup.removedCount;
    }
...
function deleteItem(index) {
    ...
    if (currentMajorTab === 'reply' && currentSubTab === 'custom') customReplies.splice(index, 1);
        ...
        if (typeof partnerPokes !== 'undefined') partnerPokes.splice(index, 1);
    }
...
function editItem(index, oldText) {
    ...
    if (currentMajorTab === 'reply' && currentSubTab === 'custom') customReplies[index] = newText.trim();
        ...
        if (typeof partnerPokes !== 'undefined') partnerPokes[index] = newText.trim();
    }
...
function _showExportUI() {
    ...
    const modules = [
        ...
        { id: '_re_pokes',    icon: ICONS.hand,      label: '拍一拍',        count: (typeof partnerPokes !== 'undefined' ? partnerPokes.length : 0), key: 'partnerPokes' },
...
function _doExport(selectedModules) {
    ...
    selectedModules.forEach(m => {
        ...
        else if (m.key === 'partnerPokes')     { libraryData.partnerPokes       = (typeof partnerPokes !== 'undefined' ? partnerPokes : []); libraryData.modules.push('pokes'); }
...
function _showGroupExportPicker(type) {
    ...
    const cfgMap = {
        ...
        pokes:    { groups: window.customPokeGroups   || [], items: (typeof partnerPokes !== 'undefined' ? partnerPokes : []), groupKey: 'customPokeGroups',   itemKey: 'partnerPokes',    moduleTag: ['pokes','pokeGroups'],     filePrefix: 'poke-groups',   label: '拍一拍',  successUnit: '条拍一拍' },
...
function _showImportUI(data) {
    ...
    const modules = [
        ...
        { id: '_ri_pokes',    icon: ICONS.hand,      label: '拍一拍',        data: (data.partnerPokes || data.customPokes), key: 'partnerPokes' },
    ...
    _showIOSheet(`导入字卡`, `文件中包含 ${modules.length} 个模块`, modules, ICONS.import, (selected, mode) => {
        ...
        try {
            ...
            let totalAdded = 0;
            const _importPokes = data.partnerPokes || data.customPokes || [];
            if (overwrite) {
                selected.forEach(m => {
                    if (m.key === 'customReplies')         { customReplies               = data.customReplies;       totalAdded += data.customReplies.length; }
                    else if (m.key === 'partnerPokes')    { if (typeof partnerPokes !== 'undefined') { partnerPokes = _importPokes; totalAdded += _importPokes.length; } }
                ...
                selected.forEach(m => {
                    ...
                    } else if (m.key === 'partnerPokes') {
                        if (typeof partnerPokes !== 'undefined') {
                            const before = partnerPokes.length;
                            partnerPokes = deduplicateContentArray([...partnerPokes, ..._importPokes]).result;
                            totalAdded += partnerPokes.length - before;
                        }
    ...
    panel.querySelector('#ba-confirm').onclick = () => {
        ...
        lines.forEach(val => {
            ...
            const isDup = currentSubTab === 'custom'
                ...
                : currentSubTab === 'pokes'
                ? (typeof partnerPokes !== 'undefined' && partnerPokes.some(r => normalizeStringStrict(r) === norm))
            ...
            else if (currentSubTab === 'pokes') { if (typeof partnerPokes !== 'undefined') { partnerPokes.push(val); newItems.push(val); } }
    ...
    if (addBtn) {
        addBtn.addEventListener('click', () => {
            ...
            if (input?.trim()) {
                ...
                let isDup = false;
                if (currentSubTab === 'pokes' && typeof partnerPokes !== 'undefined' && partnerPokes.some(r => normalizeStringStrict(r) === valNorm)) isDup = true;
                ...
                if (currentSubTab === 'pokes') { if (typeof partnerPokes !== 'undefined') partnerPokes.unshift(val); }
