...
const loadData = async () => {
    try {
        ...
        const results = await Promise.allSettled([
            ...
            localforage.getItem(getStorageKey('partnerPokes'))
        ]);
        ...
        // 加载对方的拍一拍库（独立存储）
        const savedPartnerPokes = getVal(21);
        if (savedPartnerPokes) partnerPokes = savedPartnerPokes;
        else partnerPokes = [...CONSTANTS.POKE_ACTIONS];
        ...
        setTimeout(() => {
            ...
            manageAutoSendTimer(); 
            checkEnvelopeStatus(); 
            if (typeof checkAndGeneratePartnerLetters === 'function') {
                checkAndGeneratePartnerLetters();
            }
            updateUI();
...
const saveData = async () => {
    ...
    const promises = [
        ...
        { key: 'partnerPokes',          val: () => localforage.setItem(getStorageKey('partnerPokes'), partnerPokes) },
...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    if (msg.type === 'system') {
        // 拍一拍消息有特殊处理，点击直接引用
        if (msg.subtype === 'poke') {
            const pokeMsgDiv = document.createElement('div');
            pokeMsgDiv.className = 'system-message poke-message';
            pokeMsgDiv.dataset.id = msg.id;
            
            // 消息内容
            const textSpan = document.createElement('span');
            textSpan.className = 'poke-text';
            textSpan.innerHTML = msg.text;
            pokeMsgDiv.appendChild(textSpan);
            
            // 引用图标（悬停显示）
            const replyIcon = document.createElement('span');
            replyIcon.className = 'poke-reply-icon';
            replyIcon.innerHTML = '<i class="fas fa-reply"></i>';
            pokeMsgDiv.appendChild(replyIcon);
            
            // 点击直接引用
            if (settings.replyEnabled) {
                pokeMsgDiv.style.cursor = 'pointer';
                pokeMsgDiv.onclick = (e) => {
                    e.stopPropagation();
                    currentReplyTo = {
                        id: msg.id,
                        sender: 'system',
            ...
            fragment.appendChild(pokeMsgDiv);
    ...
    if (msg.replyTo) {
        ...
        let repliedSender;
        if (msg.replyTo.isPoke) {
            repliedSender = '拍一拍';
        } else if (msg.replyTo.sender === 'user') {
            repliedSender = settings.myName || '我';
        } else {
            repliedSender = settings.partnerName || '对方';
        }
    ...
    actionsHTML += `<button class="meta-action-btn favorite-action-btn ${msg.favorited ? 'favorited' : ''}" title="${msg.favorited ? '取消收藏' : '收藏'}"><i class="fas fa-star"></i></button>`;
...
window.addMessage = addMessage;
        ...
        window.updateReplyPreview = function() {
            ...
            let senderName;
            if (currentReplyTo.isPoke) {
                senderName = '拍一拍';
            } else if (currentReplyTo.sender === 'user') {
                senderName = settings.myName || '我';
            } else {
                senderName = settings.partnerName || '对方';
            }
        ...
        window._triggerPartnerPoke = function() {
            ...
            // 使用独立的 partnerPokes，不与 customPokes 共用
            const allPokes = (typeof partnerPokes !== 'undefined' ? partnerPokes : []) || [];
            ...
            addMessage({ id: Date.now(), text: pokeText, timestamp: new Date(), type: 'system', subtype: 'poke', favorited: false, note: null });
...
if (!isBatchMode && type === 'normal') {
    ...
    window._pendingReplyTimer = null;
        ...
        function toggleBatchMode() {
            // 批量发送功能已移除UI，此函数保留作为兼容
            if (!DOMElements.batchBtn || !DOMElements.batchPreview) return;
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            // ── 检测用户最近的拍一拍消息，优先回应 ──
            const recentMsgIds = new Set(messages.slice(-99).map(m => m.id));
            const recentPokes = messages.filter(m => 
                m.type === 'system' && m.subtype === 'poke' && m.text && 
                m.text.includes(settings.myName || '我') &&
                recentMsgIds.has(m.id)
            );
            // 从所有最近的拍一拍中随机选一个引用（不再只选最后一个）
            const selectedPoke = recentPokes.length > 0 
                ? recentPokes[Math.floor(Math.random() * recentPokes.length)] 
                : null;
            const isPokeRecent = selectedPoke !== null;
            
            // 如果用户最近发了拍一拍（且在上下文距离内），有较低概率回应
            if (isPokeRecent && Math.random() < 0.2) {
                // 25% 用拍一拍回应
                if (Math.random() < 0.25) {
                    if (typeof window._triggerPartnerPoke === 'function') {
                        ...
                        }, settings.replyDelayMin + Math.random() * (settings.replyDelayMax - settings.replyDelayMin));
                    }
                    return;
                }
                // 60% 引用拍一拍消息回复
            ...
            const recentUserMsgs = settings.replyEnabled
                ? messages.filter(m => m.sender === 'user' && m.text).slice(-36)
                : [];
            for (let i = 0; i < replyCount; i++) {
                ...
                setTimeout(() => {
                    ...
                    addMessage({
                        ...
                        note: null,
                        replyTo: (i === 0 && (() => {
                            // 优先引用用户的拍一拍消息（仅当在上下文距离内）
                            if (isPokeRecent && Math.random() < 0.36) {
                                return { id: selectedPoke.id, text: selectedPoke.text, sender: 'system', isPoke: true };
                            }
                            // 否则引用普通消息
                            if (recentUserMsgs.length > 0 && Math.random() < 0.56) {
