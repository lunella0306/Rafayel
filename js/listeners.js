    ...
    return;
        ...
        function initModalListeners() {
            ...
            DOMElements.pokeModal.save.addEventListener('click', () => {
                let pokeText = DOMElements.pokeModal.input.value.trim() || `拍了拍 ${settings.partnerName}`;
                ...
                // 自动添加主语（用户名字）
                const finalPokeText = _formatPokeText(`${settings.myName} ${pokeText}`);
                addMessage({
                    id: Date.now(), text: finalPokeText, timestamp: new Date(), type: 'system', subtype: 'poke', favorited: false, note: null
                });
...
const savedCover = safeGetItem(APP_PREFIX + 'playerCover');
    ...
    const saveMusicState = () => {
        ...
        localforage.setItem(APP_PREFIX + 'musicState', state).catch(e => {
    ...
    const loadMusicState = async () => {
        try {
            const savedState = await localforage.getItem(APP_PREFIX + 'musicState');
            if (savedState) {
                if (typeof savedState.currentIndex === 'number' && savedState.currentIndex >= 0 && savedState.currentIndex < songs.length) {
                ...
                return savedState.currentTime || 0;
            }
        } catch (e) {
    ...
    function loadSong(index, restoreTime = 0) {
...
playlist.style.top = (rect.top + (player.classList.contains('collapsed') ? 65 : 155)) + 'px';
    ...
    audio.addEventListener('timeupdate', (e) => {
        ...
        // 每5秒保存一次播放进度
        if (Math.floor(currentTime) % 5 === 0 && isPlaying) {
            saveMusicState();
        }
    });
    ...
    // 初始化播放器
    (async () => {
        const savedTime = await loadMusicState();
        loadSong(currentIndex, savedTime);
        renderPlaylist();
        setupDrag();

        // 如果上次开启了播放器，刷新后显示但默认折叠
        if (settings.musicPlayerEnabled) {
            player.classList.add('visible');
            player.classList.add('collapsed'); // 默认折叠状态
        }
    })();
};
        ...
        function initCoreListeners() {
            ...
            if (headerMusicBtn) {
                headerMusicBtn.addEventListener('click', toggleMusicPlayer);
            }
